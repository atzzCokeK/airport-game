...
export function TitleScene({ muted, onToggleMute }: Props) {
  ...
  useEffect(() => {
    setMaxCleared(getMaxClearedStage())
  }, [])
    ...
    >
      ...
      >
        <svg width="56" height="34" viewBox="0 0 40 24" style={{ filter: 'drop-shadow(3px 3px 0 #1F2430)' }}>
          ...
          <rect x="7" y="9" width="26" height="6" fill="#E0644B" clipPath="url(#c1)" />
          <clipPath id="c1"><ellipse cx="20" cy="12" rx="18" ry="7" /></clipPath>
      ...
      >
        ...
        {maxCleared > 0 && (
          <button
            onClick={() => startStage(Math.min(maxCleared + 1, TOTAL_STAGES))}
            style={{
              background: '#F2B544',
              color: '#1F2430',
              border: '3px solid #1F2430',
          ...
          >
            つづきから（ステージ {Math.min(maxCleared + 1, TOTAL_STAGES)}）
          </button>
        )}

        {maxCleared > 0 && (
          <button
            onClick={() => { resetProgress(); setMaxCleared(0) }}
            style={{
      ...
      <motion.div
        ...
        style={{ display: 'flex', gap: 10, marginTop: 36 }}
      >
        {Array.from({ length: TOTAL_STAGES }).map((_, i) => (
          <div
            key={i}
            style={{
              width: i < maxCleared ? 22 : 18,
              height: i < maxCleared ? 22 : 18,
              borderRadius: i < maxCleared ? 4 : '50%',
              background: i < maxCleared ? '#2F6E6A' : '#D4C4A8',
...
function TowerSVG() {
  return (
    <svg width="130" height="160" viewBox="0 0 130 160" style={{ filter: 'drop-shadow(4px 4px 0 #1F2430)' }}>
      ...
      {/* Windows row */}
      {[45, 60, 75].map((x) => (
        <rect key={x} x={x} y={112} width={12} height={10} rx={1} fill="#D4EDFA" stroke="#1F2430" strokeWidth={1.5} />
      ))}
      {[45, 60, 75].map((x) => (
        <rect key={x + 100} x={x} y={130} width={12} height={10} rx={1} fill="#D4EDFA" stroke="#1F2430" strokeWidth={1.5} />
      ))}
      {/* Tower stem */}
      ...
      {/* Control windows */}
      {[46, 58, 70].map((x) => (
        <rect key={x} x={x} y={39} width={10} height={16} rx={1} fill="#D4EDFA" stroke="#1F2430" strokeWidth={1.5} />
      ))}
      {/* Roof antenna */}
